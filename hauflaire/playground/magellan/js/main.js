...
$(document).ready(function() {
	...
	function buildImages(data,pagename) {
		...
		(data.pages && data.pages[pagename].html) ? buildHTML(data,pagename) : false;
		
	}
	
	function buildHTML(data,pagename) {
		$.each(data.pages[pagename].html, function(key,value) {
			var this_child = $("#ipad-views li:nth-child("+key+")");
			var fname = data.pages[pagename].html[key].fontname;
			$(this_child).addClass("html").addClass(fname);
			var li_html = $(data.pages[pagename].html[key].content);
			$(this_child).prepend("<section id='font-name'>font: "+fname+"</section>").prepend(li_html);
	    });
	}
